...
class Pemeriksaan_editController extends Controller {
    ...
    public function accessRules() {
        return array(
            array('allow', // allow authenticated user to perform 'create' and 'update' actions
                'actions' => array('read', 'update','delete'),
                'users' => array('@'),
            ),
    ...
    public function actionRead() {
        ...
        foreach ($data as $dp) {
            $biaya_pembayaran = $this->getJumlahPembayaranPemeriksaan($dp['id_registrasi_pemeriksaan']);
            $data_pembayaran = $this->getPembayaranPemeriksaan($dp['id_registrasi_pemeriksaan']);
            ...
            if (count($data_pembayaran) > 0 && $biaya_pembayaran['total_biaya'] == ($biaya_pembayaran['total_bayar']+$biaya_pembayaran['potongan'])) {
                Yii::app()->db->createCommand("update registrasi_pemeriksaan set status_pembayaran=1 where id_registrasi_pemeriksaan='{$dp['id_registrasi_pemeriksaan']}'")->query();
            } else {
                Yii::app()->db->createCommand("update registrasi_pemeriksaan set status_pembayaran=0 where id_registrasi_pemeriksaan='{$dp['id_registrasi_pemeriksaan']}'")->query();
            }
        }
