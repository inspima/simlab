<?php
    ...
    class PemeriksaanController extends Controller
    {
            ...
            return array(
                array('allow', // allow authenticated user to perform 'create' and 'update' actions
                    'actions' => ['read', 'excel', 'pengirim',
                        ...
                        'CtkPel_piutang', 'Pendapatan_2', 'pascovid', 'ExcelPascovid',
                        'wgscovid', 'ExcelWgscovid', 'excelPendLab'],
                    'users' => array('@'),
                ),
        ...
        public static function getWgsocidData($awal, $akhir)
        {
            $kode_pengujian_wgs = 17092;
            $query = "SELECT
                ...
                JOIN pengujian p on p.id_pengujian=a.id_pengujian
                ...
                WHERE
                    p.kode_pengujian = '{$kode_pengujian_wgs}'
                    AND DATE_FORMAT(b.waktu_registrasi,  '%Y-%m-%d') >=  '$awal'
                    AND DATE_FORMAT(b.waktu_registrasi,  '%Y-%m-%d') <= '$akhir'
                ";

            return Yii::app()->db->createCommand($query)->queryAll();
        }

        public function actionWgscovid()
        {
            $unit = Unit::model()->findAll(array('order' => 'id_unit DESC'));
            $data = [];
            ...
            if (!empty($_POST)) {
                ...
                $awal = Yii::app()->request->getPost('awal');
                $akhir = Yii::app()->request->getPost('akhir');
                $data = PemeriksaanController::getWgsocidData($awal, $akhir);

                $this->render('wgscovid', array(
                    'unit' => $unit,
                    'pil_unit' => $pil_unit,
                    'nama_unit' => $nama_unit,
                    'awal' => $awal,
        ...
        public function actionExcelWgscovid($b, $e)
        {
            $this->layout = false;
            //$pil_unit = Yii::app()->request->getQuery('id');
            //$nama_unit = Unit::model()->findByAttributes(array('id_unit' => $pil_unit));
            ...
            $akhir = Yii::app()->getRequest()->getQuery('e');
            $data = PemeriksaanController::getWgsocidData($awal, $akhir);
            $this->render('excelWgscovid', array(
                //'id' => $id,
                //'pil_unit' => $pil_unit,
                //'nama_unit' => $nama_unit,
                'awal' => $awal,
